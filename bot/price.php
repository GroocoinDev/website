...
<?php
    ...
    for ($i = 0; $i < 5; $i++) {
        $data = $data. "<tr align='center' height='50'><td align='center'>". date("Y-m-d H:i:s", $data["result"][$i]["timestamp"] + $UTC9) ."</td><td>".$data["result"][$i]["type"]."</td><td align='right'>".number_format($data["result"][$i]["quantity"])." ".$coin."</td><td align='right'>".$data["result"][$i]["price"]. " ETH</td></tr>";
    }
    $data = $data. "</table>";
?>
<meta charset="UTF-8">
