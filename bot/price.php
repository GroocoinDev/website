...
<?php
    ...
    $outputData = "<table width='30%'>";
    $outputData = $outputData."<tr><td height='300'>";
    ...
    for ($i = 0; $i < count($data2); $i++) {
        if ($data2[$i]["market_name"] == $pair) {
            ...
            $outputData = $outputData. "[가격]";
            $outputData = $outputData. " (".$change."%)";
            $outputData = $outputData. "<br>현재 : ".$data2[$i]["last"]." ETH (".sprintf('₩ %0.2f', $data2[$i]["last"]*$ETH_PRICE).")";
            $outputData = $outputData. "<br>어제 : ".$data2[$i]["lastDayAgo"]." ETH (".sprintf('₩ %0.2f', $data2[$i]["lastDayAgo"]*$ETH_PRICE).")";
            $outputData = $outputData. "<br>볼륨 : ".number_format($data2[$i]["vol"] * $data2[$i]["last"], 5)." ETH<br><br>";
            break;
        }
    }
    
    $outputData = $outputData. "[매수]<br>금액 : ".$orderbooks->result->buy[0]->Rate." ETH<br>개수 : ".number_format($orderbooks->result->buy[0]->Quantity)." ".$coin."<br>비용 : ".number_format($orderbooks->result->buy[0]->Quantity * $orderbooks->result->buy[0]->Rate, 8)." ETH<br><br>";
    $outputData = $outputData. "[매도]<br>금액 : ".$orderbooks->result->sell[0]->Rate." ETH<br>개수 : ".number_format($orderbooks->result->sell[0]->Quantity)." ".$coin."<br>비용 : ".number_format($orderbooks->result->sell[0]->Quantity * $orderbooks->result->sell[0]->Rate, 8)." ETH";

    $outputData = $outputData. "</tr></td>";
